...
## 説明スタイル
- ユーザーは構造文脈型（トップダウン型）の学習者。骨組みが先、詳細が後でないと理解が崩れる。
- 説明は必ず **全体像 → 詳細** の順。先に「何の問題を解いてるか／このコードがシステム内でどこに位置するか」を1〜2行で示し、その後に具体を出す。
- 詳細を先に並べる説明は禁止。コード片だけ提示して終わるのも避ける。
- 階層構造を明示する（大項目 → 中項目 → 詳細）。線形に長文を流さない。
- 新しい概念・関数・ファイルを出すときは「なぜ存在するか（Why）」を先に押さえてから「何をするか（What）」「どう動くか（How）」へ。
